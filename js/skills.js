...
// Function to render projects based on category type
function loadProjects(type) {
    if (!data[type]) return;

    projectsContainer.innerHTML = data[type].map(item => `
        <div class="card">
            <div class="card-image">
                ...
                <div class="card-overlay">
                    ...
                    <div class="actions">
                        ${item.live ? `<a href="${item.live}" target="_blank" rel="noopener noreferrer">Live</a>` : ""}
                        ${item.github ? `<a href="${item.github}" target="_blank" rel="noopener noreferrer">GitHub</a>` : ""}
                        ${item.whatsapp ? `<a href="${item.whatsapp}" target="_blank" rel="noopener noreferrer">WhatsApp</a>` : ""}
                    </div>
                </div>
            </div>
        </div>
...
// Event delegation for handling tab switching on mobile and desktop
...
// Initial load for custom websites
loadProjects("custom");